...
void Yaml::Awake()
{
	...
	std::string path = pResPath->FindResPath("/engine.yaml");
	YAML::Node appConfig = YAML::LoadFile(path);
	LoadConfig(APP_TYPE::APP_ALLINONE, appConfig);
	LoadConfig(APP_TYPE::APP_LOGIN, appConfig);
	LoadConfig(APP_TYPE::APP_GAME, appConfig);
	LoadConfig(APP_TYPE::APP_SPACE, appConfig);
	LoadConfig(APP_TYPE::APP_ROBOT, appConfig);
	LoadConfig(APP_TYPE::APP_DB_MGR, appConfig);
	LoadConfig(APP_TYPE::APP_APPMGR, appConfig);

	path = pResPath->FindResPath("/bt_config.yaml");
	YAML::Node npcConfig = YAML::LoadFile(path);
	LoadConfig(NPC_TYPE::Public, npcConfig);
	LoadConfig(NPC_TYPE::Dungeon, npcConfig);
...
{
	if (_appConfigs.find(appType) != _appConfigs.end())
		return _appConfigs[appType];

	std::cout << "load config failed. appType:" << GetAppName(appType) << std::endl;
	return nullptr;
}

BtConfig* Yaml::GetConfig(NPC_TYPE npcType)
{
	if (_npcConfigs.find(npcType) != _npcConfigs.end())
		return _npcConfigs[npcType];
	return nullptr;
}
...
void Yaml::LoadConfig(const APP_TYPE appType, YAML::Node& config)
{
	...
	_appConfigs.insert(std::make_pair(appType, pYamlConfig));
}

void Yaml::LoadConfig(NPC_TYPE npcType, YAML::Node& config)
{
	std::string npcTypeName = GetNpcType(npcType);
	YAML::Node node = config[npcTypeName];
	BtConfig* pBtConfig = new BtConfig();;
	pBtConfig->rebirth = node["rebirth"].as<bool>();
	YAML::Node selector = node["selector"];
